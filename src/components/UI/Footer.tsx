...
const Footer: FC = (): JSX.Element => {
   return (
      <nav className="z-40 mt-48 flex w-full justify-center pb-7">
            ...
            <div className="flex items-center gap-3 *:size-6">
               {social.map(({ href, imageSrc, title }) => (
                  <a href={href} key={href} target="_blank" title={title}>
                     <Image src={imageSrc} alt={title} width={25} height={25} className="transition-transform hover:scale-125 dark:opacity-90 dark:invert" />
                  </a>
               ))}
            </div>
